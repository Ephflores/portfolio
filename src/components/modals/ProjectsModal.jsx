    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={prevProject} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full transition-all duration-300 hover:scale-110" aria-label="Previous project">
            <ChevronLeft className="w-5 h-5 text-cyan-400" />
          </button>
          <button type="button" onClick={nextProject} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full transition-all duration-300 hover:scale-110" aria-label="Next project">
            <ChevronRight className="w-5 h-5 text-cyan-400" />
          </button>
        </div>
      </div>
      <Card className="bg-slate-800/50 border-slate-700 overflow-hidden">
        <div className={`h-48 bg-gradient-to-br ${currentProject.color || 'from-cyan-500 to-blue-500'} relative`}>
          <div className="absolute inset-0 bg-black/20" aria-hidden />
          <div className="absolute top-4 right-4 flex gap-2">
            {currentProject.github && !currentProject.githubRestricted && (
              <a href={currentProject.github} target="_blank" rel="noopener noreferrer" className="p-2 bg-black/50 backdrop-blur-sm rounded-full hover:bg-black/70 transition-all duration-300 hover:scale-110" aria-label="GitHub">
            ...
            {currentProject.demo && !currentProject.demoRestricted && (
              <a href={currentProject.demo} target="_blank" rel="noopener noreferrer" className="p-2 bg-black/50 backdrop-blur-sm rounded-full hover:bg-black/70 transition-all duration-300 hover:scale-110" aria-label="Demo">
          ...
          <div className="pt-4 border-t border-slate-700">
            ...
            <div className="flex flex-wrap gap-2">
              {currentProject.techStack.map((tech, index) => (
                <Badge key={index} className="text-xs bg-slate-700/50 text-gray-300 border-slate-600 hover:border-cyan-500/50 transition-colors duration-300">
                  {tech}
                </Badge>
              ))}
            </div>
