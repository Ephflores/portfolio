...
import resume from '../assets/EphraimFloresResume.pdf';
...
const BentoGrid = ({ openModal }) => {
  ...
  const getSizeClass = (size, id) => {
    switch (size) {
      case 'large':
        if (id === 'projects') {
          return 'col-span-12 md:col-span-6 lg:col-span-4 row-span-2';
        }
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h2 className={`font-bold text-white mb-2 ${
                  tile.size === 'large' ? 'text-3xl md:text-5xl' : 'text-xl md:text-2xl'
                }`}>
                  {tile.title}
                </h2>
                {tile.subtitle && (
