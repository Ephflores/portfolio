...
const ExperienceModal = () => {
  ...
  const ExperienceCard = ({ exp, type }) => {
    const isExpanded = expandedCards[`${type}-${exp.id}`];
        ...
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <button
                onClick={() => toggleCard(`${type}-${exp.id}`)}
      ...
      <div>
        ...
        <div className="space-y-4">
          {technicalExperience.map((exp) => (
            <ExperienceCard key={exp.id} exp={exp} type="tech" />
          ))}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {otherExperience.map((exp) => (
              <ExperienceCard key={exp.id} exp={exp} type="other" />
            ))}
          </div>
        </div>
      )}
